...
import { downloadAndSaveImage } from "../server/image-service";
...
async function fetchCollectionCards(col: Collection): Promise<Card[]> {
  ...
  for (const slug of slugsToTry) {
    ...
    console.log(`[collection] ${slug}: ${cards.length} ürün`);
    return cards;
  }
  console.log(`[collection] ${col.slug}: 0 ürün (bulunamadı)`);
  return [];
}
...
async function main() {
  console.log("=== Veteriner import başladı ===");
  ...
  for (const col of COLLECTIONS) {
    const cards = await fetchCollectionCards(col);
    if (cards.length === 0) continue;
    ...
    await pool(cards, 3, async (card) => {
      ...
      try {
        ...
        created++;
        if (created % 20 === 0) console.log(`[progress] ${created} ürün eklendi...`);
      } catch (err: any) {
        console.log(`[hata] ${card.name}: ${err.message}`);
      }
    });
  }

  console.log(`=== Bitti. Eklendi: ${created}, Atlandı: ${skipped} ===`);
  process.exit(0);
}
