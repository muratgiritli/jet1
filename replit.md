...
- **PayTR Sanal POS (Online Kredi Kartı)**: PayTR iframe API entegrasyonu. Müşteri checkout'ta "Online Kredi Kartı" seçerse modal içinde PayTR güvenli iframe açılır (kart, taksit, BKM destekli). Sipariş `paymentStatus="pending"` ile yaratılır, stok düşer; `/api/paytr/callback` ödeme onayında `paid` olur, loyalty puan ve admin SMS gönderilir; başarısız olursa sipariş iptal edilir ve stok iade edilir. `/odeme-sonuc` sayfası başarı/başarısız sonucu gösterir. Frontend 3sn'de bir polling ile durumu kontrol eder. Secrets: `PAYTR_MERCHANT_ID`, `PAYTR_MERCHANT_KEY`, `PAYTR_MERCHANT_SALT`. PayTR panelinden bildirim URL'si `https://www.jetgo.pet/api/paytr/callback` olarak ayarlanmalıdır.
