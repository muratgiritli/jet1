...
- **Karanlık Mod**: `ThemeContext` (`client/src/contexts/ThemeContext.tsx`) toggles `dark` class on `<html>`, persists to `localStorage` (`jetgo_theme`), respects `prefers-color-scheme`. Sun/Moon toggle button in `Header.tsx` next to login. Tailwind already configured with `darkMode: ["class"]` and `:root` / `.dark` HSL variables in `index.css`.
...
- **Hover-Lens Zoom**: `ImageZoom.tsx` desktop-only magnifier (160px circle, 3× zoom) follows cursor on product images. Mobile retains existing pinch/double-tap modal.
