...
- **PayTR Sanal POS (Online Kredi Kartı)**: PayTR iframe API entegrasyonu. Müşteri checkout'ta "Online Kredi Kartı" seçerse `https://www.paytr.com/odeme/guvenli/{token}` adresine tam yönlendirme yapılır (PayTR'ın CSP/X-Frame-Options kısıtları nedeniyle iframe yerine redirect kullanılır). Sipariş `paymentStatus="pending"` ile yaratılır, stok düşer; `/api/paytr/callback` ödeme onayında `paid` olur, loyalty puan ve admin SMS gönderilir; başarısız olursa sipariş iptal edilir ve stok iade edilir. PayTR ödeme sonrası kullanıcıyı `/odeme-sonuc?orderId=X` adresine geri yönlendirir; bu sayfa başarı/başarısız durumunu gösterir ve sepeti temizler. Secrets: `PAYTR_MERCHANT_ID`, `PAYTR_MERCHANT_KEY`, `PAYTR_MERCHANT_SALT`. Opsiyonel: `PAYTR_TEST_MODE=1` ile test modu. PayTR panelinden bildirim URL'si `https://www.jetgo.pet/api/paytr/callback` olarak ayarlanmalıdır.
