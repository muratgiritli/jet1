...
- **User Panel (/hesabim)**: 9-tab profile with: Profilim, Para Puanlarım, Siparişlerim (with order detail expansion, product images, and one-click reorder), Harcama Özeti (spending summary with monthly bar charts), Favorilerim, Adreslerim, Evcil Hayvanlarım, Bildirimler, Güvenlik (password change + KVKK-compliant account deletion).
- **Admin Panel**: Enhanced with 7-section tabbed navigation: Dashboard (real-time stats, revenue charts, top products, low stock alerts), Yönetim (existing product/order/campaign management), Kuponlar (coupon CRUD - create/edit/delete/toggle active), Müşteri (customer list with search, edit, WhatsApp contact), Bildirim (bulk SMS broadcast to customers), Banner (CRUD for promotional banners with image upload), Raporlama (payment methods, order status, top customers, monthly revenue, mahalle bazlı sipariş raporu). Navigation uses sticky pill-shaped buttons below header.
