...
## Google Local Inventory feed (`/google-local-inventory.xml`) is opt-in via store_code
Separate from the product feed; links products to a PHYSICAL store for local-inventory ads / free local listings. Store-scoped via `reqStore`.
- The linchpin is `merchant.storeCode` — Google Business Profile location code, an EXTERNAL must-match. If empty the feed emits a valid but EMPTY channel (0 items, logs MISSING); this is the safe opt-in default that keeps the other 8 stores untouched until they set their own code.
- Rows mirror the product feed's EXACT product set + `g:id` (=product.id) so Google can match by id; never emit a local row for a product missing from the product feed. Local availability is only `in_stock`/`out_of_stock` (no `preorder` locally → stock<=0 = out_of_stock). Includes `g:price` (+`g:sale_price` on discount), same esc/clean/fmtPrice helpers, Cache-Control 3600 like the product feed.
- **Why:** Merchant local inventory requires a registered Business Profile store; the feed alone can't create one. **How to apply:** owner sets the real Business Profile store code in the live admin Merchant section, then pastes the local feed URL into Merchant Center. Testing locally: direct-SQL writes to `<store>:merchant` need a restart (or 60s) because of the in-memory merchant cache.
