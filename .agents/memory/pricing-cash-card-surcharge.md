...
- JETGO-ONLY per-product override (byte-identical for the other 8 stores): store "jetgo" can set the non-cash rate PER PRODUCT via app_setting `jetgo:product_surcharge_overrides` (JSON `{ "<productId>": <percent> }`). When present on jetgo, order/cart/display use per-line `sum(price*qty*effRate)` (override rate for listed products, else the store-wide single rate), rounded ONCE. Any other store, or jetgo with no overrides, falls through to the ORIGINAL single-rate `subtotal*rate` expression untouched — this is how the other stores stay byte-identical. Client resolver `client/src/hooks/useSurchargeRate.ts` (`parseSurchargeOverrides` gated on `CURRENT_STORE.id==="jetgo"`, `effectiveSurchargeRate(productId, base, overrides)`); `product_surcharge_overrides` is whitelisted into `/api/public-settings`. Admin GET/PATCH `/api/admin/product-surcharge-overrides` are jetgo-gated via `adminStoreId(req)` (reads `?store=` / `body.store`, NOT host) and read-merge-write ONE entry (empty percent clears just that id; 0 is a real stored value). NOTE: this override key is deliberately NOT in `STORE_SCOPED_SETTING_KEYS` (it's a raw `jetgo:`-prefixed key, not part of the base/override fallback drift-guard set).
