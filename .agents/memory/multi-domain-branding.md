...
description: How one codebase serves multiple branded sites (flagship Enuygun/enuygunpet.com + atakumpetshop etc.) and where rebranding must happen
---
...
- The default/flagship store's internal `id` is still `"jetgo"` (all DB scoping keys off it), but its BRAND is
  Enuygun and its domain is `https://www.enuygunpet.com`. `jetgomarket.com` is kept in the flagship's
...
- Wrapping a literal in `brandify()` is always safe: the flagship now yields Enuygun/enuygunpet.com, other
