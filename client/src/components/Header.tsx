    ...
    <>
      ...
      <nav className="sticky top-[52px] z-[9998]" style={{ backgroundColor: "#7c4dff" }}>
        <div className="max-w-6xl mx-auto px-1 md:px-2">
          <ul className="flex items-center justify-center gap-0.5 md:gap-2 py-1 md:py-1.5 flex-nowrap whitespace-nowrap" data-testid="nav-categories">
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className={`px-1.5 md:px-6 py-0.5 md:py-1.5 text-[10px] md:text-base font-medium transition-colors rounded-md md:rounded-lg ${
                      isHighlight
