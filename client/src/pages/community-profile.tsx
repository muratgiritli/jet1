...
export default function CommunityProfilePage() {
  ...
  const [form, setForm] = useState({ name: "", city: "", dogName: "", bio: "", avatar: "" });
  ...
  const startEdit = () => {
    if (!me) return;
    setForm({ name: me.name, city: me.city, dogName: me.dogName, bio: me.bio, avatar: me.avatar });
    setEditing(true);
  };
    ...
    <CommunityLayout>
      ...
      <div className="px-4 pt-6 text-center" data-testid="user-panel">
        ...
        <h1 className="mt-3 text-lg font-extrabold text-[#1C1B1F]" data-testid="text-profile-name">
          {isLoggedIn ? me?.name : "Misafir"}
        </h1>
        <p className="text-[13px] text-[#6B6573] mt-1">
          {isLoggedIn
            ? `${me?.dogName || "Poodle"} · ${me?.city || "Türkiye"} · @${me?.username}`
        ...
        {isLoggedIn && profileQuery.data && (
          <p className="mt-2 text-[12px] text-[#6B6573]">
            {profileQuery.data.member.postCount} gönderi · {profileQuery.data.member.followers} takipçi · {profileQuery.data.member.following} takip
          </p>
        )}
        {isLoggedIn ? (
          <div className="mt-4 flex justify-center gap-2 flex-wrap">
            <button
              type="button"
              onClick={startEdit}
              className="h-10 px-4 rounded-full border border-[#D9D0EC] text-sm font-semibold text-[#5B4B86]"
            ...
            {me?.isAdmin && (
              <Link
                href="/yp-admin"
                className="h-10 px-4 rounded-full bg-[#8E7CC3] text-white text-sm font-semibold inline-flex items-center"
            ...
            <button
              type="button"
              onClick={() => void logout()}
              className="h-10 px-4 rounded-full text-sm font-semibold text-[#8A8494]"
          ...
          <div className="mt-4 flex justify-center gap-2">
            ...
            <Link
              href="/yp-giris?tab=register&redirect=/profil"
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {SOCIAL_PHOTOS.map((photo) => (
              <button
                key={photo}
                type="button"
                onClick={() => setForm((f) => ({ ...f, avatar: photo }))}
                className={`rounded-lg overflow-hidden ring-2 ${form.avatar === photo ? "ring-[#8E7CC3]" : "ring-transparent"}`}
              >
                <img src={photo} alt="" className="aspect-square w-full object-cover" />
              </button>
            ))}
          </div>
          <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Ad" className="w-full h-10 rounded-xl border border-[#E4DCF3] bg-white px-3 text-sm" />
          <input value={form.city} onChange={(e) => setForm((f) => ({ ...f, city: e.target.value }))} placeholder="Şehir" className="w-full h-10 rounded-xl border border-[#E4DCF3] bg-white px-3 text-sm" />
          <input value={form.dogName} onChange={(e) => setForm((f) => ({ ...f, dogName: e.target.value }))} placeholder="Köpeğinin adı" className="w-full h-10 rounded-xl border border-[#E4DCF3] bg-white px-3 text-sm" />
          <textarea value={form.bio} onChange={(e) => setForm((f) => ({ ...f, bio: e.target.value }))} placeholder="Kısa bio" rows={3} className="w-full rounded-xl border border-[#E4DCF3] bg-white p-3 text-sm" />
            ...
            {tab === "saved" && (
              <div className="grid grid-cols-3 gap-1">
                ...
                {savedQuery.data?.posts.length === 0 && (
                  <p className="col-span-3 text-[13px] text-[#6B6573] py-4">Henüz kayıtlı gönderin yok.</p>
                )}
              </div>
            )}
            {tab === "followers" && (
              <MemberList members={profileQuery.data?.followers || []} empty="Henüz takipçin yok." />
            )}
            {tab === "following" && (
              <MemberList members={profileQuery.data?.following || []} empty="Henüz kimseyi takip etmiyorsun." />
            )}
          </section>
        </>
      )}
