...
export default function CommunityAdminPage() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-white flex items-center justify-center text-sm text-[#6B6573]">Yükleniyor…</div>;
  }
  ...
  if (!isLoggedIn || !me?.isAdmin) {
    return (
      <div className="min-h-screen bg-white max-w-lg mx-auto px-4 py-10 text-center">
        ...
        <p className="mt-2 text-sm text-[#6B6573]">Bu sayfa yalnızca yöneticiler içindir.</p>
  ...
  return (
    <div className="min-h-screen bg-white text-[#1C1B1F] max-w-lg mx-auto" data-testid="admin-panel">
      <SEO title="Yönetim — YourPoodle" description="YourPoodle içerik yönetimi." canonical={`${SITE_DOMAIN}/yp-admin`} />
      <header className="sticky top-0 z-40 bg-white/95 border-b border-[#EEEAF4] px-3 h-12 flex items-center justify-between">
        <h1 className="text-[15px] font-extrabold">Yönetim</h1>
        <Link href="/" className="text-[12px] font-semibold text-[#8E7CC3]">Akışa dön</Link>
      </header>
      <div className="px-3 pt-4 grid grid-cols-3 gap-2 text-center">
        <Stat label="Üye" value={overview.data?.users} />
        <Stat label="Gönderi" value={overview.data?.posts} />
        <Stat label="Rapor" value={overview.data?.reportsOpen} />
      </div>
      ...
      <div className="px-3 py-4 space-y-2 pb-10">
        {tab === "users" && (users.data?.users || []).map((user) => (
          <div key={user.id} className="rounded-2xl border border-[#F1EDF6] p-3 flex items-center gap-2">
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-semibold truncate">{user.name} {user.isAdmin && <span className="text-[#8E7CC3]">· yönetici</span>}</p>
              <p className="text-[11px] text-[#6B6573]">@{user.username} · {user.city} · {user.postCount} gönderi</p>
            </div>
          </div>
        ))}
        ...
        {tab === "reports" && (reports.data?.reports || []).map((report) => (
          <div key={report.id} className="rounded-2xl border border-[#E4DCF3] p-3">
            <p className="text-[12px] font-semibold">{report.reporterName} · {report.targetType} · {report.status}</p>
...
function ModRow({
  ...
  return (
    <div className="rounded-2xl border border-[#F1EDF6] p-3">
      <p className="text-[12px] font-semibold">{title} {hidden && <span className="text-[#8E7CC3]">· gizli</span>}</p>
