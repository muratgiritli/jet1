...
export default function CommunityPostPage() {
  ...
  const report = async () => {
    if (!requireAuth()) return;
    await socialSend("POST", `/api/social/posts/${id}/report`, { reason: "İncelenmesini istiyorum." });
    setError("Rapor iletildi.");
  };

  if (!isLoading && !data) {
    return (
      <CommunityLayout>
        <div className="px-4 py-12 text-center text-sm text-[#6B6573]">Gönderi bulunamadı.</div>
      </CommunityLayout>
    );
  }
  ...
  return (
    <CommunityLayout>
      <SEO
        title={data ? `${data.post.author} paylaşımı — YourPoodle` : "Gönderi"}
      ...
      <section className="px-3 pt-1">
        <div className="flex items-center justify-between">
          <h2 className="text-[13px] font-bold">{data?.comments.length || 0} yorum</h2>
        ...
        <ul className="mt-2 space-y-2">
          {(data?.comments || []).map((comment) => (
            <li key={comment.id} className="rounded-2xl bg-[#FAF8FD] p-3">
              <Link href={`/uye/${comment.username}`} className="text-[12px] font-semibold">
                {comment.author} <span className="font-normal text-[#6B6573]">· {comment.time}</span>
              </Link>
